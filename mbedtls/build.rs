...
fn main() {
    let env_components = env::var("DEP_MBEDTLS_PLATFORM_COMPONENTS")
        .or(env::var("DEP_ESP_IDF_PLATFORM_COMPONENTS"))
        .unwrap();
    let mut sys_platform_components = HashMap::<_, HashSet<_>>::new();
    for mut kv in env_components
        .split(",")
        .map(|component| component.splitn(2, "="))
    {
    ...
    if env::var("DEP_MBEDTLS_PLATFORM_COMPONENTS").is_ok() {
